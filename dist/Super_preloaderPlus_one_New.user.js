	...
	(function () {
	  ...
	  function ensureExhentaiReloadButton(img) {
	    ...
	    if (!button || !button.isConnected) {
	      ...
	      button.innerHTML = "<img alt=\"\" src=\"".concat(exhentaiReloadIconDataUri, "\">");
	  ...
	  async function retryExhentaiImage(img, explicitToken, explicitPageUrl) {
	    let options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
	    if (!img || exhentaiRetryingImages.has(img)) return {
	      ok: false,
	      reason: 'busy'
	    };
	    const manual = !!options.manual;
	    const showButtonOnStop = options.showButtonOnStop !== false;
	    if (!manual && img.dataset && img.dataset.spExhentaiRetryStopped === '1') return {
	      ok: false,
	      reason: 'stopped'
	    };
	    ...
	    if (!token || !retryPageUrl || !exhentaiSPagePattern.test(retryPageUrl)) return {
	      ok: false,
	    ...
	    if (!manual && tried.has(retryUrl)) {
	      logger.warn("[Super-preloader]", 'ExHentai nl retry stopped because the same token was already tried', retryUrl);
	    ...
	    try {
	      ...
	      if (!data.src && !data.nlToken) {
	        logger.warn("[Super-preloader]", 'ExHentai nl retry stopped because no replacement source or token was found', retryPageUrl);
	        img.dataset.spExhentaiRetryStopped = '1';
	        if (showButtonOnStop) showExhentaiReloadButton(img);
	        return {
	          ok: false,
	      ...
	      if (data.src) {
	        ...
	        img.src = data.src;
	        scheduleExhentaiFailedImageCheck(img, 1200, {
	          autoRetry: false
	        });
	      } else {
	        scheduleExhentaiFailedImageCheck(img, 50, {
	          autoRetry: false
	        });
	      }
	      return {
	      ...
	      logger.warn("[Super-preloader]", 'ExHentai nl retry request failed', e);
	      if (showButtonOnStop) showExhentaiReloadButton(img);
	      return {
	        ok: false,
	  ...
	  async function startExhentaiRetryTraversal(img, explicitToken, explicitPageUrl) {
	    ...
	    img.dataset.spExhentaiRetryStopped = '0';
	    setExhentaiReloadButtonLoading(img);
	    try {
	  ...
	  function bindExhentaiImageRetry(img, pageUrl, options, explicitToken) {
	    ...
	    scheduleExhentaiFailedImageCheck(img, 250, {
	      autoRetry: true
	    });
	    if (options.renameIds && img.id === 'img') {
	      img.id = "sp-exhentai-img-".concat(options.pageIndex, "-").concat(options.index);
	    }
	  }
	  function scheduleExhentaiFailedImageCheck(img, delay) {
	  ...
	  function handleExhentaiNlCall(token) {
	    ...
	    if (img) {
	      bindExhentaiImageRetry(img, location.href, {
	        renameIds: false,
	        pageIndex: 0,
	        index: 0
	      }, token);
	  ...
	  function installExhentaiNlOverride() {
	    ...
	    const replacement = function replacement(token) {
	  ...
	  function bindExhentaiNlTrigger(elem, root, pageUrl, options) {
	    ...
	    if (options.renameIds && elem.id === 'loadfail') {
	      elem.id = "sp-exhentai-loadfail-".concat(options.pageIndex, "-").concat(options.index);
	    }
	  }
	  function enableExhentaiNlRetry(root, pageUrl) {
	    let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	  ...
	  function installExhentaiCurrentPageNlRetry() {
	    ...
	    document.addEventListener('click', event => {
	      ...
	      event.stopPropagation();
	      event.stopImmediatePropagation();
	      bindExhentaiNlTrigger(elem, document, location.href, {
	        renameIds: false,
	        pageIndex: 0,
	        index: 0
	      });
	      const img = findExhentaiImageByToken(document, token) || findExhentaiPrimaryImage(document);
	      if (img) bindExhentaiImageRetry(img, elem.dataset.spExhentaiPageUrl || location.href, {
	        renameIds: false,
	        pageIndex: 0,
	        index: 0
	      }, token);
	    ...
	    if (document.readyState === 'loading') {
	      ...
	      document.addEventListener('DOMContentLoaded', patchCurrentPage, {
	        once: true
	      });
	    } else {
	      patchCurrentPage();
	  ...
	  if (document.readyState === 'loading') {
	    document.addEventListener('DOMContentLoaded', startSuperPreloader, {
	      once: true
	    });
	  } else {
	    startSuperPreloader();
